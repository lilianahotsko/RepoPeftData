...
def main() -> None:
    ...
    ap.add_argument("--per-step-input", default="auto",
                    ...
                    help="What the GRU ingests per commit step. 'auto' "
                         "(default) reads the value stored in the checkpoint "
                         "(falls back to 'diff' for older checkpoints).")
    args = ap.parse_args()
    ...
    # Resolve per-step-input mode (auto -> read from checkpoint metadata).
    per_step_input = args.per_step_input
    if per_step_input == "auto":
        _meta = torch.load(str(args.checkpoint), map_location="cpu",
                           weights_only=False)
        per_step_input = (_meta.get("per_step_input")
                          or _meta.get("args", {}).get("per_step_input")
                          or "diff")
        del _meta
        print(f"[per-step-input] auto-detected: {per_step_input}", flush=True)
