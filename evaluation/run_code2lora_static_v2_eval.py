#!/usr/bin/env python3
"""Per-snapshot task-metric evaluation for **v2 Code2LoRA-direct** checkpoints.

Loads a checkpoint produced by ``hypernetwork/train_code2lora_static_v2.py``
(``head.{best,latest,epN}.pt``), reconstructs the :class:`Code2LoRAHead`,
wraps the base LLM with :class:`LoRA` modules, and walks every (repo, snapshot)
in a suite -- scoring every QnA attached to that snapshot with the LoRA
freshly generated from ``head(repo_state_embedding)``.

This is the direct-projection counterpart to
``evaluation/run_code2lora_gru_v2_eval.py`` for the *static* (non-GRU) variant.
There is **no GRU rollout** -- each (repo, commit) snapshot is scored
independently using the precomputed ``repo_state_embedding`` for that
snapshot. Output schema matches ``evaluation/run_baselines_v2.py`` and
``run_code2lora_gru_v2_eval.py`` so plots overlay cleanly:
...
* **Per-(repo, commit) incremental atomic write** (``tmp.write_text + os.replace``)
  -- a timeout never loses more than the one snapshot currently in flight.
* **Resumable**: if the output JSON already exists and is not ``finalized``,
  finished (repo, commit) groups are skipped on restart.
* Uses **precomputed repo-state embeddings** straight out of the v2
  snapshots parquet -- no encoder forward pass during eval.

Usage::

    python evaluation/run_code2lora_static_v2_eval.py \\
        --checkpoint $CKPT_DIR/CODE2LORA_STATIC_V2/.../head.best.pt \\
        --snapshots-dir $SCRATCH/REPO_DATASET/code2lora_snapshots_hf \\
        --suite cr_val \\
        --output-dir $CKPT_DIR/CODE2LORA_STATIC_EVAL_V2/<run>/ \\
...
def _load_v2_static_ckpt(
    ckpt_path: Path,
    base_model_name: str,
    target_modules: List[str],
    device: torch.device,
    dtype: torch.dtype = torch.bfloat16,
) -> Tuple[torch.nn.Module, Code2LoRAHead, List[Any], Any]:
    """Rebuild (base_model, head, specs, tokenizer) from a v2 static ckpt.

    The static-V2 trainer saves ``{"state_dict", "config", "type_dims", "args"}``.
    This loader is tolerant of GRU-style keys (``head_state``, ``head_config``)
    too, in case the file was produced by a future variant.
    """
    print(f"[load] ckpt={ckpt_path}", flush=True)
    state = torch.load(str(ckpt_path), map_location="cpu", weights_only=False)
    head_cfg = state.get("config") or state.get("head_config") or {}
    head_state = state.get("state_dict") or state.get("head_state")
    if head_state is None:
        raise SystemExit(
            f"[load] checkpoint at {ckpt_path} has neither 'state_dict' "
            f"nor 'head_state'; keys present: {sorted(state.keys())}")
    ...
    input_dim = int(head_cfg.get("input_dim", 2048))
    head = Code2LoRAHead(
        input_dim=input_dim,
        type_dims=type_dims,
    ...
    print(f"[load] base+LoRA ready, head rank={rank} alpha={alpha} "
          f"hidden={head_hidden} input_dim={input_dim}; "
...
def evaluate_suite(
    *,
    ...
    print(f"\n[suite {suite_name}] loading snapshots parquet "
          f"{snapshots_parquet}", flush=True)
    snap_rows = load_snapshot_rows(snapshots_parquet,
                                   in_repo_splits=in_repo_splits_to_score)
    # Group by repo and sort within repo by commit_index for determinism.
    ...
    print(f"[suite {suite_name}] {len(all_repos)} repos, "
          f"{sum(len(v) for v in rows_by_repo.values())} snapshots total",
          flush=True)
    ...
    # Predict how many scored (repo, commit) groups exist for our shard.
    n_total_groups = 0
    for repo, rows in rows_by_repo.items():
        for row in rows:
    ...
    print(f"[suite {suite_name}] shard {shard_i+1}/{num_shards} DONE  "
          ...
          f"({len(all_samples):,} qnas, {len(per_commit_records):,} snapshots)",
          flush=True)
    return summary
...
def main() -> None:
    ...
    ap.add_argument("--checkpoint", required=True,
                    help="Path to head.{best,latest,epN}.pt produced "
                         "by train_code2lora_static_v2.py.")
    ap.add_argument("--snapshots-dir",
                    ...
                    help="Dir containing commits/{ir_val,ir_test,cr_val,cr_test}.parquet "
                         "and qna/{ir_val,ir_test,cr_val,cr_test}.parquet.")
    ...
    ap.add_argument("--qnas-per-commit-limit", type=int, default=8,
                    help="Cap QnAs per snapshot (matches v2 trainer eval cap).")
    ...
    out_path = out_dir / f"static_v2_{args.suite}{shard_suffix}.json"

    base_model, head, specs, tok = _load_v2_static_ckpt(
    ...
    if not snapshots_parquet.exists():
        raise SystemExit(f"missing snapshots parquet: {snapshots_parquet}")
    ...
    evaluate_suite(
        ...
        base_model=base_model, head=head, specs=specs,
