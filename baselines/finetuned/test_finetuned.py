#!/usr/bin/env python3
"""
Evaluate fine-tuned Qwen2.5-Coder on split JSON.

Usage:
    python baselines/finetuned/test_finetuned.py --model-path $SCRATCH/TRAINING_CHECKPOINTS/FINETUNED/final --split cr_test
    python baselines/finetuned/test_finetuned.py --model-path $SCRATCH/TRAINING_CHECKPOINTS/FINETUNED/final --split ir_test
"""
...
def main():
    ap = argparse.ArgumentParser(description="Evaluate fine-tuned baseline on split JSON")
    ...
    for start in range(0, n, batch_size):
        ...
        preds = invoke_batch(
            ...
            device=args.device, pad_token_id=tok.pad_token_id,
        )
        ...
        n_eval = len(entries)
        _res = {
            "method": "finetuned", "split": args.split,
