#!/bin/bash
#SBATCH --job-name=dump_preds_ir_test
#SBATCH --output=slurm_logs/dump_preds_ir_test_%A_%a.out
#SBATCH --error=slurm_logs/dump_preds_ir_test_%A_%a.err
...
# context-injection baselines) for a sampled subset of the ir_test suite.
# The subset is driven by a {repo_id, commit_sha} JSONL produced by
# `visualize_gru_ir_test_qnas.py --write-selected-keys`.
#
...
#       --write-selected-keys $SCRATCH/IR_TEST_PRED_DUMP/selected_keys.jsonl
#
#   # 2. submit all 5 dump jobs:
#   KEYS_FILE=$SCRATCH/IR_TEST_PRED_DUMP/selected_keys.jsonl \
#       OUT_DIR=$SCRATCH/IR_TEST_PRED_DUMP \
#       sbatch --array=0-4 scripts/slurm/dump_ir_test_predictions.sh
#
#   # 3. rebuild the report with prediction text & contexts attached:
#   python visualize_gru_ir_test_qnas.py \
#       --predictions-dir $SCRATCH/IR_TEST_PRED_DUMP \
...
KEYS_FILE="${KEYS_FILE:?KEYS_FILE must point to the selected_keys.jsonl produced by visualize_gru_ir_test_qnas.py --write-selected-keys}"
...
QNAS_DIR_ROOT="${QNAS_DIR_ROOT:-$SCRATCH/REPO_DATASET/code2lora_snapshots_hf}"
...
C2L_STATIC_CKPT="${C2L_STATIC_CKPT:-$CKPT_DIR/CODE2LORA_STATIC/h100_v2_static_3ep_run5/head.ep2.pt}"
C2L_GRU_CKPT="${C2L_GRU_CKPT:-$CKPT_DIR/CODE2LORA_GRU/h100_v2_gru_3ep/gru_head.best.pt}"
...
echo "===== Dump predictions [task ${IDX}] ====="
echo "Method        : $METHOD"
