...
\method{} achieves 63.8\% exact match on CR evaluation, substantially outperforming pretrained baselines (45.7\%), full fine-tuning (51.4\%), retrieval-augmented generation (39.7\%), and in-context learning (42.2\%).
The shared-basis variant (\method{}-PAW) achieves 64.1\% CR EM with $\sim$8.5$\times$ fewer hypernetwork parameters.
...
We train on all 409 IR repositories (5,192 test pairs).
...
RAG ($k$=3) & 39.7 & 0.516 & 0.555 & 42.1 & 0.544 & 0.581 \\
ICL (3-shot) & 42.2 & 0.559 & 0.617 & 45.3 & 0.596 & 0.642 \\
Oracle Context & 47.5 & 0.614 & 0.649 & 48.7 & 0.627 & 0.659 \\
\midrule
\multicolumn{7}{l}{\emph{Trained (prefix $\to$ target)}} \\
\midrule
FFT & 51.4 & 0.695 & 0.678 & 55.9 & 0.727 & 0.714 \\
Single LoRA & 45.6 & 0.640 & 0.639 & 48.2 & 0.669 & 0.666 \\
Per-repo LoRA & --- & --- & --- & 64.0 & 0.801 & 0.788 \\
\textbf{\method{}} & \textbf{63.8} & \textbf{0.784} & \textbf{0.777} & \textbf{66.2} & \textbf{0.806} & \textbf{0.795} \\
\method{}-PAW & 64.1 & 0.785 & 0.779 & 65.8 & 0.805 & 0.792 \\
\midrule
\multicolumn{7}{l}{\emph{Trained (oracle + prefix $\to$ target)}} \\
\midrule
FFT + Oracle & 48.3 & 0.675 & 0.655 & 51.7 & 0.695 & 0.683 \\
sLoRA + Oracle & 44.4 & 0.635 & 0.630 & 45.8 & 0.652 & 0.649 \\
pLoRA + Oracle & --- & --- & --- & \todoinline{XX.X} & \todoinline{X.XX} & \todoinline{X.XX} \\
\textbf{\method{} + Oracle} & 63.1 & 0.778 & 0.772 & 65.3 & 0.802 & 0.791 \\
\method{}-PAW + Oracle & 64.1 & 0.787 & 0.780 & \textbf{67.0} & \textbf{0.812} & \textbf{0.801} \\
\bottomrule
\end{tabular}
\caption{Main results on \benchmark{}. EM = Exact Match (\%), EditSim = Edit Similarity, CodeBLEU. Best non-oracle results per column in \textbf{bold}; best oracle results in \textbf{bold}. Per-repo LoRA is N/A for CR (no adapter for unseen repos). \todo{pLoRA + Oracle pending (eval jobs queued).}}
...
\begin{enumerate}[nosep]
    ...
    RAG (39.7\%) and ICL (42.2\%) perform \emph{below} the pretrained baseline (45.7\%) on CR.
    ...
    All trained methods outperform context-based methods, confirming that adapting model parameters is more effective than input augmentation for repository-level tasks.
    ...
    At 64.0\% EM on IR (5,192 examples across 409 repos), per-repo LoRA demonstrates the value of repository-specific adaptation, but \method{} still outperforms it (66.2\%) while requiring no per-repo training.
\end{enumerate}
...
\method{} + Oracle & 63.1 & \todoinline{XX.X} \\
\midrule
\multicolumn{3}{c}{\emph{IR Test}} \\
\midrule
\method{} & 66.2 & \todoinline{XX.X} \\
\method{} + Oracle & 65.3 & \todoinline{XX.X} \\
\bottomrule
\end{tabular}
\caption{Oracle cross-evaluation ablation for \method{}. Training with oracle context and testing without it slightly hurts performance ($-$0.7 CR, $-$0.9 IR), suggesting the model becomes dependent on oracle signals absent at test time. \todo{Fill ``w/ Oracle'' test columns (requires oracle context at inference).}}
...
150 (37\%) & 61.5 \\
200 (49\%) & 62.2 \\
300 (73\%) & \todoinline{XX.X} \\
409 (100\%) & 63.8 \\
623$^\dagger$ & 63.6 \\
\bottomrule
\end{tabular}
\caption{Effect of training set size on CR test performance. $^\dagger$Expanded dataset including repos with $<$30 QnA pairs.}
...
Shared-Basis (PAW) & 64.1 & 65.8 & \todoinline{X}M \\
\bottomrule
\end{tabular}
\caption{Comparison of hypernetwork architectures. PAW achieves comparable performance with $\sim$8.5$\times$ fewer parameters.}
...
Per-repo LoRA was evaluated across all 409 IR repositories (5,192 test pairs), providing representative coverage.
However, cross-repo per-repo LoRA evaluation is inherently impossible (no adapter exists for unseen repos).
The scaling analysis is limited to 623 repositories due to data availability; extending to 1,000+ repos would strengthen the scaling law claims.
...
Evaluated on \benchmark{}, a benchmark of 560 Python repositories with 73K assertion completion tasks across cross-repo and in-repo settings, \method{} achieves 63.8\% exact match on cross-repo evaluation---substantially outperforming pretrained (45.7\%), fine-tuned (51.4\%), and context-based (RAG: 39.7\%, ICL: 42.2\%) baselines.
Notably, \method{} outperforms even per-repo LoRA training (64.0\%) on in-repo evaluation (66.2\%) despite requiring zero per-repo training data.
...
\midrule
3 & 39.7 & 0.516 & 0.555 & 42.1 & 0.544 & 0.581 \\
5 & 37.5 & 0.486 & 0.527 & 41.0 & 0.524 & 0.559 \\
10 & 36.3 & 0.469 & 0.509 & 39.4 & 0.521 & 0.574 \\
\bottomrule
\end{tabular}
...
\midrule
3 & 42.2 & 0.559 & 0.617 & 45.3 & 0.596 & 0.642 \\
5 & 42.2\textsuperscript{*} & 0.545 & 0.616 & 44.9 & 0.590 & 0.638 \\
\bottomrule
\end{tabular}
\caption{ICL results for varying shot counts. \textsuperscript{*}5-shot CR evaluated on n=2,700 (partial run due to context length limits).}
\label{tab:icl_shots}
\end{table}
